...
static const struct hda_verb wm8860g_capture_init_verbs[] = {
	...
	{NID_PORT_D, AC_VERB_SET_AMP_GAIN_MUTE,
		SET_AMP_STEREO | 0x26},
	{NID_PORT_H, AC_VERB_SET_AMP_GAIN_MUTE,//{NID_PORT_H, AC_VEca0132_codec_probeRB_SET_AMP_GAIN_MUTE,
		SET_AMP_STEREO | 0x26},
...
#endif
static int probe_wm8860g(struct hda_codec *codec)
{
	struct wm8860_spec *spec;
...
static void wm8860g_remove(struct hda_codec *codec)
{
	struct wm8860_spec *spec = codec->spec;
...
static int wm8860g_probe(struct hda_codec *codec, const struct hda_device_id *id)
{
	int err;
	err = probe_wm8860g(codec);

	if (err < 0) {
		wm8860g_remove(codec);
		return err;
	}
...
static const struct hda_device_id snd_hda_id_wolfson[] = {
	...
	HDA_CODEC_ID(0x1aec8800,"WM8860"),
	{ /* end */ }
};
